 ...
 * Checks for updates, applies them via ZIP download, then restarts.
...
// Files that must NEVER be overwritten (preserves user config)
const PROTECTED_FILES = ['settings.js', 'config.js', '.env'];

// Files to copy from the fresh download
...
async function fetchCommitInfo() {
  try {
    ...
    let formattedDate = '';
    if (commitDate) {
      formattedDate = formatDate(new Date(commitDate));
    }

    // Extract added .js files from plugins as "new commands"
    const newCommands = [];
    files.forEach(f => {
